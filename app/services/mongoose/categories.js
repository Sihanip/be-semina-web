...
const { BadRequestError, NotFoundError } = require("../../erros");
...
const creteCategories = async (req) => {
  const { name } = req.body;
...
const updateCategories = async (req) => {
  ...
  const check = await Categories.findOne({
    name,
    organizer: req.user.organizer,
    _id: { $ne: id }, //$ne adalah fungsi untuk mencari seluruh data apakah ada yg dplikat/tidak
  });
